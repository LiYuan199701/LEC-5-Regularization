...
# Max norm weight regularization Text 15 p. 290
...
def DNNfit(Xtrain, ytrain):  
    ...
    model = Sequential()
# first hidden layer
    model.add(Dense(Nnodes, input_dim=Nfeatures, activation='relu', kernel_constraint = unit_norm()))
...
ax.boxplot(PEG_vals)
ax.set_title(Language + ' - Weight Regularization')
ax.set_xlabel('PEG')
ax.set_ylabel('PE')
plt.show()    
